...
print("\n7. 발전소별 발전량 통계")
print("-" * 30)
plant_stats = df.groupby('발전구분')['발전량(kWh)'].agg([
...
# 1) 발전소별 총 발전량
plt.subplot(3, 3, 1)
plant_total = df.groupby('발전구분')['발전량(kWh)'].sum().sort_values(ascending=False)
plt.bar(range(len(plant_total)), plant_total.values)
plt.title('발전소별 총 발전량', fontsize=12)
plt.xlabel('발전소')
plt.ylabel('총 발전량 (kWh)')
...
# 2) 시간대별 평균 발전량
plt.subplot(3, 3, 2)
hourly_avg = df.groupby('시간')['발전량(kWh)'].mean()
...
# 3) 발전량 분포 (히스토그램)
plt.subplot(3, 3, 3)
non_zero_generation = df[df['발전량(kWh)'] > 0]['발전량(kWh)']
...
plt.subplot(3, 3, 4)
df['월'] = df['일자'].dt.month
monthly_generation = df.groupby('월')['발전량(kWh)'].sum()
...
plt.subplot(3, 3, 5)
# 0이 아닌 발전량만 사용
non_zero_df = df[df['발전량(kWh)'] > 0]
sns.boxplot(data=non_zero_df, x='발전구분', y='발전량(kWh)')
plt.title('발전소별 발전량 분포 (0 제외)', fontsize=12)
plt.xticks(rotation=45, ha='right')

# 7) 0 발전량 비율
plt.subplot(3, 3, 7)
zero_ratio = df.groupby('발전구분').apply(lambda x: (x['발전량(kWh)'] == 0).sum() / len(x) * 100)
...
for plant in df['발전구분'].unique():
    ...
    operation_rate = (plant_data['발전량(kWh)'] > 0).sum() / len(plant_data) * 100
...
# 전체 통계 요약
total_generation = df['발전량(kWh)'].sum()
avg_generation = df['발전량(kWh)'].mean()
print(f"\n전체 발전량 통계:")
print(f"  - 총 발전량: {total_generation:,.2f} kWh")
print(f"  - 평균 발전량: {avg_generation:.2f} kWh")
print(f"  - 무발전 시간 비율: {(df['발전량(kWh)'] == 0).sum() / len(df) * 100:.1f}%")
